...
class Feeder_hard(Dataset):
    ...
    def load_data(self):
        ...
        if self.split == 'train':
            ...
            # self.label = np.where(np.logical_and(npz_data['y_train'] > 67 , npz_data['y_train'] < 77))[1]
            # print(self.data.shape)
            self.label = np.where(npz_data['y_train'] > 0)[1]
            self.ind = np.logical_and(self.label>67,self.label<77)
            self.label = self.label[self.ind] - 68
            # self.ind = np.where(npz_data['y_train'] > 0)[0]
            # print(npz_data['y_train'])
            # print(self.label.shape)
            # print(self.ind.shape)
            ...
            self.ind = np.logical_and(self.label>67,self.label<77)
            self.label = self.label[self.ind] - 68
            # self.label = np.where(np.logical_and(npz_data['y_test'] > 67 , npz_data['y_test'] < 77))[1]
            # self.ind = np.where(np.logical_and(npz_data['y_test'] > 67 , npz_data['y_test'] < 77))[0]
            self.data = self.data[self.ind,:,:]
            self.sample_name = ['test_' + str(i) for i in range(len(self.data))]
        else:
