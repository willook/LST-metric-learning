...
def split_dataset(skes_joints, label, performer, camera, evaluation, save_path):
    ...
    print("save the data file")
    np.savez('NTU120_CS.npz', 
...
def get_indices(performer, camera, evaluation='CS'):
    ...
    if evaluation == 'CS':  # Cross Subject (Subject IDs)
        ...
        train_ids = [1, 2, 4, 5, 8, 9, 13, 14, 15, 16, 17, 
                     18, 19, 25, 27, 28, 31, 34, 35, 38, 45, 
                     46, 47, 49, 50, 52, 53, 54, 55, 56, 57, 
                     58, 59, 70, 74, 78, 80, 81, 82, 83, 84, 
                     85, 86, 89, 91, 92, 93, 94, 95, 97, 98, 
                     100, 103]
        test_ids = []
        for id in range(1, 107):
